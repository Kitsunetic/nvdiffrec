...
class DLMesh(torch.nn.Module):
    ...
    def tick(self, glctx, target, lgt, opt_material, loss_fn, iteration):
        ...
        if self.FLAGS.laplace == "absolute":
            reg_loss += (
                regularizer.laplace_regularizer_const(self.mesh.v_pos, self.mesh.t_pos_idx)
                * self.FLAGS.laplace_scale
                * (1 - t_iter)
            )
        elif self.FLAGS.laplace == "relative":
            reg_loss += (
                regularizer.laplace_regularizer_const(self.mesh.v_pos - self.initial_guess.v_pos, self.mesh.t_pos_idx)
                * self.FLAGS.laplace_scale
                * (1 - t_iter)
            )
