...
class DMTet:
    ...
    def __call__(self, pos_nx3, sdf_n, tet_fx4):
        ...
        faces = torch.cat(
            (
                torch.gather(
                    input=idx_map[num_triangles == 1], dim=1, index=self.triangle_table[tetindex[num_triangles == 1]][:, :3]
                ).reshape(-1, 3),
                torch.gather(
                    input=idx_map[num_triangles == 2], dim=1, index=self.triangle_table[tetindex[num_triangles == 2]][:, :6]
                ).reshape(-1, 3),
            ),
            dim=0,
        )
...
class DMTetGeometry(torch.nn.Module):
    ...
    def tick(self, glctx, target, lgt, opt_material, loss_fn, iteration):
        ...
        img_loss = img_loss + loss_fn(
            buffers["shaded"][..., 0:3] * color_ref[..., 3:], color_ref[..., 0:3] * color_ref[..., 3:]
        )
        ...
        reg_loss += (
            torch.mean(buffers["occlusion"][..., :-1] * buffers["occlusion"][..., -1:]) * 0.001 * min(1.0, iteration / 500)
        )
