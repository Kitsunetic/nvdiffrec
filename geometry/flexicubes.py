...
class FlexiCubes:
    ...
    def construct_voxel_grid(self, res):
        ...
        cubes = (base_cube_f.unsqueeze(0) + torch.arange(coords.shape[0], device=self.device).unsqueeze(1) * 8).reshape(-1)
    ...
    def _compute_vd(self, x_nx3, surf_cubes_fx8, surf_edges, s_n, case_ids, beta_fx12, alpha_fx8, gamma_f, idx_map, grad_func):
        ...
        for num in torch.unique(num_vd):
            ...
            if grad_func is not None:
                with torch.no_grad():
                    ...
                    verts_group_pos = torch.index_select(input=zero_crossing, index=verts_group_idx.reshape(-1), dim=0).reshape(
                        -1, num.item(), 7, 3
                    )
                    ...
                    vd.append(
                        self._solve_vd_QEF(
                            (verts_group_pos - v0) * curr_mask, normals_bx7x3 * curr_mask, verts_centroid - v0.squeeze(2)
                        )
                        + v0.reshape(-1, 3)
                    )
        ...
        if grad_func is not None:
            ...
            beta_group = torch.gather(input=beta_fx12.reshape(-1), dim=0, index=edge_group_to_cube * 12 + edge_group).reshape(
                -1, 1
            )
    ...
    def _triangulate(
        ...
        if grad_func is not None:
            ...
            ) * torch.index_select(input=quad_gamma, index=torch.tensor(2, device=self.device), dim=1)
            gamma_13 = torch.index_select(
                input=quad_gamma, index=torch.tensor(1, device=self.device), dim=1
            ) * torch.index_select(input=quad_gamma, index=torch.tensor(3, device=self.device), dim=1)
        if not training:
            ...
            vd_02 = (
                ...
                + torch.index_select(input=vd_quad, index=torch.tensor(2, device=self.device), dim=1)
            ) / 2
            vd_13 = (
                torch.index_select(input=vd_quad, index=torch.tensor(1, device=self.device), dim=1)
                + torch.index_select(input=vd_quad, index=torch.tensor(3, device=self.device), dim=1)
            ) / 2
            weight_sum = (gamma_02 + gamma_13) + 1e-8
            vd_center = ((vd_02 * gamma_02.unsqueeze(-1) + vd_13 * gamma_13.unsqueeze(-1)) / weight_sum.unsqueeze(-1)).squeeze(
                1
            )
    ...
    def _tetrahedralize(
        ...
        """
        inside_verts_idx = mapping_inside_verts[surf_edges[edge_indices.reshape(-1, 4)[:, 0]].reshape(-1, 2)[s_edges < 0]]
        if not training:
        ...
        inside_cubes_center_idx = (
            torch.arange(inside_cubes_center.shape[0], device=inside_cubes.device) + vertices.shape[0] + inside_verts.shape[0]
        )

        surface_n_inside_cubes = surf_cubes | inside_cubes
        edge_center_vertex_idx = torch.ones(((surface_n_inside_cubes).sum(), 13), dtype=torch.long, device=x_nx3.device) * -1
