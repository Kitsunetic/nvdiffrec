...
def load_mtl(fn, clear_ks=True):
    ...
    for mat in materials:
        ...
        if "bump" in mat:
            mat["normal"] = texture.load_texture2D(
                os.path.join(mtl_path, mat["bump"]), lambda_fn=lambda x: x * 2 - 1, channels=3
            )
...
def merge_materials(materials, texcoords, tfaces, mfaces):
    ...
    for tex in textures:
        if tex in materials[0]:
            tex_data = torch.cat(
                tuple(util.scale_img_nhwc(mat[tex].data, tuple(max_res)) for mat in materials), dim=2
    ...
    for fi in range(len(tfaces)):
        ...
        for vi in range(3):
            ...
            if not (matIdx in new_tverts[ti]):  # create new vertex
                new_tverts_data.append(
                    [(matIdx + texcoords[ti][0]) / s_coeff[1], texcoords[ti][1] / s_coeff[0]]
                )  # Offset texture coodrinate (x direction) by material id & scale to local space. Note, texcoords are (u,v) but texture is stored (w,h) so the indexes swap here
