...
def compute_edges(attr_idx, return_inverse=False):
    with torch.no_grad():
        ...
        sorted_edges = torch.cat((torch.gather(all_edges, 1, order), torch.gather(all_edges, 1, 1 - order)), dim=-1)
...
def auto_normals(imesh):
    ...
    v_nrm = torch.where(
        util.dot(v_nrm, v_nrm) > 1e-20, v_nrm, torch.tensor([0.0, 0.0, 1.0], dtype=torch.float32, device="cuda")
    )
