...
def prepare_batch(target, bg_type="black"):
    ...
    target["img"] = torch.cat(
        (torch.lerp(background, target["img"][..., 0:3], target["img"][..., 3:4]), target["img"][..., 3:4]), dim=-1
    )
    ...
    if mlp:
        ...
        if FLAGS.random_textures or init_mat is None:
            ...
            ks_map_opt = texture.create_trainable(
                np.concatenate((ksR, ksG, ksB), axis=2), FLAGS.texture_res, not FLAGS.custom_mip, [ks_min, ks_max]
            )
        else:
        ...
        if FLAGS.random_textures or init_mat is None or "normal" not in init_mat:
            normal_map_opt = texture.create_trainable(
                np.array([0, 0, 1]), FLAGS.texture_res, not FLAGS.custom_mip, [nrm_min, nrm_max]
            )
        else:
            normal_map_opt = texture.create_trainable(
...
def optimize_mesh(
    ...
    learning_rate = (
        ...
        if isinstance(FLAGS.learning_rate, list) or isinstance(FLAGS.learning_rate, tuple)
        else FLAGS.learning_rate
    )
    learning_rate_pos = (
        learning_rate[0] if isinstance(learning_rate, list) or isinstance(learning_rate, tuple) else learning_rate
    )
    learning_rate_mat = (
        learning_rate[1] if isinstance(learning_rate, list) or isinstance(learning_rate, tuple) else learning_rate
    )
        ...
        if FLAGS.local_rank == 0:
            ...
            if display_image or save_image:
                result_image, result_dict = validate_itr(
                    glctx, prepare_batch(next(v_it), FLAGS.background), geometry, opt_material, lgt, FLAGS
                )
    ...
    if os.path.splitext(FLAGS.ref_mesh)[1] == ".obj":
        ...
        if os.path.isfile(os.path.join(FLAGS.ref_mesh, "poses_bounds.npy")):
            ...
            dataset_train = DatasetNERF(
                os.path.join(FLAGS.ref_mesh, "transforms_train.json"), FLAGS, examples=(FLAGS.iter + 1) * FLAGS.batch
            )
